<script>
  function data() {
    return {
      ...
      currentInfo: {},
      init() {
        this.currentInfo = this.information[0];
        setInterval(() => {
          this.currentIndex = (this.currentIndex + 1) % this.information.length;
          this.currentInfo = this.information[this.currentIndex];
        }, 2000);
      },
    };
  }
...
<div x-data="data" x-init="init" style="height: 100vh">
      ...
      <div class="services columns is-multiline is-gapless is-marginless has-background-dark">
        ...
        <div class="column is-5 is-clipped">
          <div class="has-text-light has-padding-30 is-full-height is-flex is-flex-direction-column is-justify-content-space-between">
            <p class="is-size-3 has-text-right" x-text="currentInfo.id"></p>
            <div>
              <h4 class="is-size-4 has-text-light" x-text="currentInfo.titulo"></h4>
              <div class="is-flex is-align-items-center is-half-width">
                @include('partials.decorator', ['color'=>'#F2F2F2'])
              </div>
              <p class="is-size-6" x-text="currentInfo.mensaje"></p>
            </div>
          </div>
        </div>
      </div>
